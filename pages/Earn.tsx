...
import { 
  ...
  Bitcoin, Coins, QrCode, RefreshCw, X, Receipt, ArrowUpCircle, ArrowDownCircle, AlertTriangle,
  ArrowLeft, TrendingUp, Clock, Info, Lock, Unlock, Calendar
} from 'lucide-react';
...
function Earn() {
  ...
  const handleStake = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (recentStakes && recentStakes.length > 0) {
        const lastStake = recentStakes[0];
        const lastStakeTime = new Date(lastStake.created_at);
        const currentTime = new Date();
        const timeDiff = (currentTime.getTime() - lastStakeTime.getTime()) / 1000 / 60; // Convert to minutes

        // If less than 5 minutes have passed and the amount is the same
        if (timeDiff < 5 && lastStake.amount === parseFloat(stakingAmount)) {
          throw new Error(
            'Você já realizou um staking com este valor nos últimos 5 minutos. ' +
            'Por favor, aguarde ou escolha um valor diferente.'
          );
        }
      }
  ...
  const handleWithdrawEarnings = async (positionId: string) => {
    try {
      ...
      const lastWithdrawal = new Date(position.last_withdrawal_date);
      const now = new Date();
      const daysSinceLastWithdrawal = Math.floor((now.getTime() - lastWithdrawal.getTime()) / (1000 * 60 * 60 * 24));

      if (daysSinceLastWithdrawal < 7) {
        throw new Error('Você só pode retirar os rendimentos uma vez por semana');
      }

      // Calculate weekly earnings
      const weeklyRate = position.period === 30 ? 0.023 : position.period === 60 ? 0.026 : 0.03;
      const weeklyEarnings = position.amount * weeklyRate;

      // Update wallet balance
      const { error: walletError } = await supabase
        .from('wallets')
        .update({
          fastcoin_balance: (parseFloat(walletData.fastcoin_balance) + weeklyEarnings).toString(),
        })
        .eq('user_id', user.id);
      ...
      const { error: stakingError } = await supabase
        .from('staking_positions')
        .update({ last_withdrawal_date: now.toISOString() })
        .eq('id', positionId);

      if (stakingError) throw stakingError;

      alert(`Rendimento semanal de ${weeklyEarnings.toFixed(2)} FASTC retirado com sucesso!`);
      fetchWalletData();
      fetchStakingPositions();
    } catch (error) {
              ...
              {stakingPositions.map((position) => (
                <div key={position.id} className="bg-navy-800 rounded-lg p-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    {/* Próximo Pagamento e Data de Desbloqueio */}
                    <div>
                      <div className="text-sm text-gray-400 mb-1">Próximo Pagamento</div>
                      <div className="font-semibold">
                        {formatDate(getNextPaymentDate(position.last_withdrawal_date).toISOString())}
                      </div>
                      <div className="text-sm text-emerald-500 mt-2">
                        Em {getRemainingTime(getNextPaymentDate(position.last_withdrawal_date))}
                      </div>
                      <div className="text-sm text-gray-400 mt-2">
                        Desbloqueio: {formatDate(position.end_date)}
                      </div>
                    </div>
                        ...
                        {position.status === 'ACTIVE' && (
                          <div className="flex flex-col sm:flex-row gap-2 mt-2">
                            <button
                              ...
                              disabled={!canWithdrawEarnings(position.last_withdrawal_date) || withdrawing}
                              className={`px-3 py-1.5 rounded-lg flex items-center justify-center gap-2 text-sm ${
                                canWithdrawEarnings(position.last_withdrawal_date)
                              ...
                              title={
                                canWithdrawEarnings(position.last_withdrawal_date)
                                  ...
                                  : 'Aguarde 7 dias desde a última retirada'
                              }
                            >
                              <ArrowUpCircle size={16} />
                              {withdrawing ? 'Processando...' : 'Retirar'}
                            </button>
                            ...
                            <button
                              ...
                              title={
                                isUnlockable(position.end_date)
                                  ? 'Desbloquear Staking'
                                  : 'Aguarde até a data de desbloqueio'
                              }
                            >
                              {isUnlockable(position.end_date) ? (
                                <Unlock size={16} />
